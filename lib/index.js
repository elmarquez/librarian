 ...
 */
function init(argv) {
    console.info(log.info, 'Initializing library');
...
function update(argv) {
    console.info(log.info, 'Updating library index');
    return lib
        .getLibraryRoot(argv.path)
        .then((root) => config.load(root).then((cfg) => [root, cfg]))
        .then((arr) => lib.getFileTreeState(arr[0]).then((state) => [...arr, state]))
        .then((arr) => lib.getFileTreeChanges(arr[0], arr[2]))
        .then(function (arr) {
            console.info(arr);
            console.info(log.success, 'Done');
        })
        .catch((err) => console.error(log.error, err));
}
